...
export default function Home() {
  return <main><ScrollReveal/><BackToTop/>
    ...
    </div><nav className="mobile-nav" aria-label="Mobile navigation"><a href="#services">Services</a><a href="#about">About</a><a href="#process">Process</a><a href="#work">Work</a><a href="#contact">Contact</a></nav></header>
    ...
    <section className="cta" id="contact"><div className="container cta-grid"><div className="cta-copy"><p className="eyebrow">LET&apos;S BUILD SOMETHING GREAT</p><h2>Ready to Take Your Business <em>Digital?</em></h2><p>Tell us what you want to build. We&apos;ll turn the idea into a practical roadmap and a product made to grow.</p><p className="contact-note"><b>Worldwide delivery</b> · Flexible MVR and USD payment options</p><div className="contact-row"><a href={`mailto:${siteConfig.email}`}>✉ {siteConfig.email}</a><a href={`https://wa.me/${siteConfig.phone.replace(/\D/g, "")}`}>◉ {siteConfig.phone}</a><span>◎ Worldwide</span></div></div><ContactForm email={siteConfig.email}/></div></section>
